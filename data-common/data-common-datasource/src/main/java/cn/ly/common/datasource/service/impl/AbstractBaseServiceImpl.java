package cn.ly.common.datasource.service.impl;

import cn.ly.common.datasource.mapper.CommonBaseMapper;
import cn.ly.common.datasource.service.IBaseService;
import cn.ly.common.datasource.service.handle.CRUDGeneratorFactory;
...
public abstract class AbstractBaseServiceImpl<T> implements IBaseService<T> {
    ...
    public int add(T t, String... ignoreFields) {
        String insertSql = CRUDGeneratorFactory.builder(dataSourceType).insertSql(t, null);
        return commonBaseMapper.insertMethod(insertSql);
    }

    @Autowired
    private CRUDGeneratorFactory crudGeneratorFactory;

    @Override
    public int addWithUid(T t) {
        String insertSql = CRUDGeneratorFactory.builder(dataSourceType).insertSqlUseUid(t);
        return commonBaseMapper.insertMethod(insertSql);
    }

    @Override
    public int addWithUid(T t, String... ignoreFields) {
        String insertSql = CRUDGeneratorFactory.builder(dataSourceType).insertSqlUseUid(t, ignoreFields);
        return commonBaseMapper.insertMethod(insertSql);
    }
